              ...
              {supportHistory.length > 0 ? supportHistory.map((obs) => (
                <div
                  ...
                  style={{
                    ...
                    borderLeft: `3px solid ${obs.performance_rating === 'Thriving' ? '#22c55e' :
...
export const TeachersScreen: React.FC = () => {
  ...
  const handleRejectSelected = async () => {
    ...
    if (!window.confirm(`Delete ${selectedTeacherIds.size} selected teacher(s)? This cannot be undone.`)) return;
  ...
  // Clear selected teachers when leaving the 'new' tab or when the list of new teachers changes
  useEffect(() => {
    setSelectedTeacherIds(new Set());
  ...
  const columns = useMemo<ColumnDef<TeacherRow>[]>(
    () => [
      ...
      ...(filterStatus === 'new' ? [{
        ...
        header: ({ table }: { table: any }) => {
          ...
          const allSelected = allRows.length > 0 && allRows.every((row: any) => selectedTeacherIds.has(row.original.id));
          return (
            <input
              type="checkbox"
              checked={allSelected}
        ...
        cell: ({ row }: { row: any }) => (
          <input
            type="checkbox"
            checked={selectedTeacherIds.has(row.original.id)}
            onChange={(e) => {
              e.stopPropagation();
            ...
            {info.row.original.needs_review && (
              <>
                ...
                <button
                  ...
                  onClick={() => {
                    if (window.confirm(`Delete teacher "${info.row.original.name}"?`)) {
                      handleReject(info.row.original.id);
                    }
                  }}
    ...
    [setShowColumnMenu, provisioningIds, handleAcknowledge, isBulkEditMode, filterStatus, selectedTeacherIds]
  );
