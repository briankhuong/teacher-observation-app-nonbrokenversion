...
> = ({ observationMeta, onBack, isOnline, isSyncing, setIsSyncing, headerVisible }) => {
  ...
  const [batchResult, setBatchResult] = useState<{
    changed: { id: string; number: string; title: string }[];
    unchanged: { id: string; number: string; title: string }[];
  } | null>(null);
  ...
  const executeBatchPolish = async () => {
    ...
    try {
      const freshCandidates = indicators
        .filter(ind => ind.commentText.trim().length > 3 && !ind.aiPendingReview && !ind.ocrPendingReview)
        .map(ind => ({ id: ind.id, number: ind.number, title: ind.title, text: ind.commentText }));
      if (freshCandidates.length === 0) {
        alert("No unpolished comments found – all indicators are either empty, already polished, or need OCR review.");
        setIsAiPolishing(false);
        return;
      }
      const results = await polishBatchWithGroq(freshCandidates);
      const changed: { id: string; number: string; title: string }[] = [];
      const unchanged: { id: string; number: string; title: string }[] = [];
      setIndicators(prev => prev.map(ind => {
        const polishedText = results[ind.id];
        if (!polishedText) {
          unchanged.push({ id: ind.id, number: ind.number, title: ind.title });
          return ind;
        }
        const isAlreadyReviewing = ind.aiPendingReview;
        const historyToKeep = isAlreadyReviewing ? ind.originalCommentText : ind.commentText;
        const hasActualChange = polishedText.trim() !== ind.commentText.trim();
        if (hasActualChange) {
          changed.push({ id: ind.id, number: ind.number, title: ind.title });
          return {
            ...ind,
            originalCommentText: historyToKeep,
  ...
  const handlePreviewPolishAll = async () => {
    ...
    try {
      ...
      const polishedIndicators = indicators.map(ind => {
        ...
        if (pText) {
          const textChanged = pText.trim() !== ind.commentText.trim();
          const historyToPreserve = ind.aiPendingReview ? ind.originalCommentText : ind.commentText;
          if (textChanged) {
            return {
              ...ind,
              originalCommentText: historyToPreserve,
              commentText: pText,
              aiPendingReview: true
            };
          } else {
            return { ...ind, aiPendingReview: false, originalCommentText: null };
          }
        }
        return ind;
      });
  ...
  const handlePolishWithAi = async (targetIndex: number) => {
    ...
    try {
      ...
      setIndicators(prev => prev.map((ind, i) => {
        ...
        const isAlreadyReviewing = ind.aiPendingReview;
        const historyToKeep = isAlreadyReviewing ? ind.originalCommentText : ind.commentText;
        const hasActualChange = polished.trim() !== ind.commentText.trim();
        if (hasActualChange) {
          return {
            ...ind,
            originalCommentText: historyToKeep,
  ...
  return (
    <div className="workspace-root">
                ...
                <div className="canvas-card">
                  <>
                      ...
                      >
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <button
                            type="button"
                            className="btn"
                            onClick={() => handlePolishWithAi(activeIndex)}
                    ...
                    >
                      ...
                      {active.aiPendingReview && active.originalCommentText !== null && active.originalCommentText !== undefined && (
                        <div style={{
                          padding: "12px",
                          background: "#020617",
                            ...
                            <div style={{ display: "flex", gap: "6px" }}>
                              <button
                                ...
                                onClick={() => updateIndicator(activeIndex, {
                                  commentText: active.originalCommentText || active.commentText,
                                  aiPendingReview: false,
                                  originalCommentText: null
                                })}
                              >
      ...
      {/* Batch Polish Result Modal */}
      {showBatchResultModal && batchResult && (
        <div className="scratchpad-backdrop">
          <div className="scratchpad-modal" style={{ maxWidth: 520, display: "flex", flexDirection: "column", maxHeight: "90vh" }}>
            <div className="scratchpad-header">
              <div>
                <div className="scratchpad-title">✨ Batch Polish Results</div>
                <div className="scratchpad-sub">
                  {batchResult.changed.length} indicator(s) polished • {batchResult.unchanged.length} unchanged
                </div>
              </div>
            </div>
            <div style={{ padding: 16, overflowY: "auto", flexGrow: 1 }}>
              {batchResult.changed.length > 0 && (
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#4ade80", marginBottom: 8 }}>
                    ✅ Polished & Review Needed
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {batchResult.changed.map(c => (
                      <div key={c.id} style={{
                        background: "rgba(34, 197, 94, 0.1)",
              ...
              {batchResult.unchanged.length > 0 && (
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#94a3b8", marginBottom: 8 }}>
                    ℹ️ Already Polished / No Changes
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {batchResult.unchanged.map(c => (
                      <div key={c.id} style={{
              ...
              {batchResult.changed.length === 0 && batchResult.unchanged.length === 0 && (
                <p style={{ fontSize: 13, color: "var(--text-muted)" }}>No indicators were processed.</p>
              )}
            </div>
            <div style={{
              padding: 16,
...
interface IndicatorRowProps {
  ...
  handlePolishWithAi: (targetIndex: number) => void;
  startRecording: () => void;
