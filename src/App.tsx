...
const NewObservationForm: React.FC<NewObservationFormProps> = ({
  ...
  // OneDrive and email‑search state
  const [teacherEmail, setTeacherEmail] = useState("");
  const [emailSearchStatus, setEmailSearchStatus] = useState<"idle" | "searching" | "no_match" | "found">("idle");
  const [emailSearchResults, setEmailSearchResults] = useState<{ school_name: string; campus: string; worksheet_url: string }[]>([]);
  ...
  // Load teacher folder for OneDrive browsing
  useEffect(() => {
    if (!user) return;
    (async () => {
  ...
  const handleEmailLookup = async () => {
    if (!teacherEmail.trim() || !user) return;
    ...
    if (!data || data.length === 0) {
      setEmailSearchStatus("no_match");
    } else {
      const unique = data.filter((v, i, a) => a.findIndex(t => t.worksheet_url === v.worksheet_url) === i);
      setEmailSearchResults(unique);
              ...
              <div style={{ position: 'relative' }}>
                <div className="input-group">
                  ...
                  <button
                    ...
                    disabled={!teacherEmail.trim() || emailSearchStatus === "searching"}
                    onClick={handleEmailLookup}
                  >
                    {emailSearchStatus === "searching" ? "..." : "🔍"}
                  </button>
                </div>
                {/* Email input for search */}
                <input
                  className="input"
                  type="email"
                  value={teacherEmail}
                  onChange={(e) => setTeacherEmail(e.target.value)}
                  placeholder="Teacher email for search"
                  style={{ ...darkInputStyle, marginTop: '8px' }}
                />
                {emailSearchStatus === "no_match" && (
                  <div style={{ fontSize: '12px', color: '#fca5a5', marginTop: '6px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(239, 68, 68, 0.1)', padding: '6px 10px', borderRadius: '4px', border: '1px solid rgba(239, 68, 68, 0.2)' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      ...
                      <span style={{ cursor: 'pointer', fontWeight: 'bold', fontSize: '18px' }} onClick={() => setEmailSearchStatus("idle")}>×</span>
                    </div>
                  </div>
                )}
                {emailSearchStatus === "found" && (
                  <div className="lookup-picker">
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', padding: '0 4px' }}>
                      ...
                      <span style={{ cursor: 'pointer', color: '#64748b' }} onClick={() => setEmailSearchStatus("idle")}>×</span>
                    </div>
                    {emailSearchResults.map((res, i) => (
                      <div
                        key={i}
